...
// Create backup
const backupPath = './finalComponentList_before_size_fix.csv';
...
for (let i = 1; i < componentLines.length; i++) {
    ...
    // Determine system based on component category
    let system = 'Ship_Components'; // Default
    ...
    const recipeKey = `${system}_${recipeComponentName}`;
    ...
    // Log progress for Heat Sink specifically
    if (componentName === 'Heat Sink') {
        heatSinkCount++;
        if (heatSinkCount <= 5) {
...
console.log(`Updated ${heatSinkCount} Heat Sink components`);
console.log('Progressive complexity now correctly applied based on size!'); 